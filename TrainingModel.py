    ...
    with tf.Session() as sess:
        ...
        for epoch_idx in range(max_epoch):
            for batch_idx in range(train_size // batch_size):
                batch_image, batch_label_op = do.get_batch_data(train_data, batch_size)
                batch_label = list(sess.run(batch_label_op))
            ...
            batch_image, batch_label = do.get_batch_data(train_data, batch_size)
            batch_label = list(sess.run(batch_label_op))
            feed_dict = {image:batch_image, label:batch_label}
        ...
        vgg_model.save_npy(sess, sys.argv[4])
